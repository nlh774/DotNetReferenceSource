    ...
    public class List<T> : IList<T>, System.Collections.IList, IReadOnlyList<T>
    {
        ...
        public List(IEnumerable<T> collection)
        {
            ...
            if (c != null)
            {
                ...
                {
                    _items = new T[count];  //新建数组容量
                    c.CopyTo(_items, 0);    //复制原序列到本序列
                    _size = count;          
                }
            }
            //原序列为空
        ...
        {
            //若子元素为null,那么仅当父元素含有null元素时才返回true
            if ((Object)item == null)
            {
        ...
        public void Insert(int index, T item)
        {
            ...
            if (_size == _items.Length) EnsureCapacity(_size + 1);
            //不使用for逐个后移，而是使用Array.Copy()自插入位置后移一位，之后赋值给插入位置的元素
            if (index < _size)
            {
        ...
        public void RemoveAt(int index)
        {
            ...
            if (index < _size)
            {
                //将原数组待删元素之后的剩余数组复制到待删元素即其后，覆盖待删元素即其后，并将列表最后一个位置的设为默认空值
                //不使用for逐个后移，而是使用Array.Copy()自插入位置后移一位，之后赋值给插入位置的元素
